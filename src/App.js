      ...
      <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #1e3a8a 0%, #3b82f6 50%, #0ea5e9 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
        <div style={{ maxWidth: '500px', width: '100%', background: 'white', borderRadius: '16px', padding: '2rem', boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }}>
          ...
          <div style={{ display: 'flex', gap: '1rem', marginBottom: '2rem' }}>
            <button onClick={() => { setShowRegister(false); setError(''); }} style={{ flex: 1, padding: '12px', background: !showRegister ? 'linear-gradient(135deg, #0070f3, #3b82f6)' : '#f1f5f9', color: !showRegister ? 'white' : '#1e3a8a', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '14px', fontWeight: '600', transition: 'all 0.3s' }}>Ingresar</button>
            <button onClick={() => { setShowRegister(true); setError(''); }} style={{ flex: 1, padding: '12px', background: showRegister ? 'linear-gradient(135deg, #0070f3, #3b82f6)' : '#f1f5f9', color: showRegister ? 'white' : '#1e3a8a', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '14px', fontWeight: '600', transition: 'all 0.3s' }}>Registrarse</button>
          </div>

          <div style={{ display: 'flex', gap: '8px', marginBottom: '2rem' }}>
            <button onClick={() => { setView('adminLogin'); setError(''); }} style={{ flex: 1, padding: '12px', background: '#10b981', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '13px', fontWeight: '600', transition: 'all 0.3s' }}>👨‍💼 Admin</button>
          </div>
          ...
          {!showRegister ? (
            <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <input type="text" placeholder="Usuario" value={loginForm.username} onChange={(e) => setLoginForm({ ...loginForm, username: e.target.value })} required style={{ padding: '12px', borderRadius: '8px', border: '2px solid #e2e8f0', fontSize: '14px', transition: 'border 0.3s', outline: 'none' }} onFocus={(e) => e.target.style.borderColor = '#0070f3'} />
              <input type="password" placeholder="Contraseña" value={loginForm.password} onChange={(e) => setLoginForm({ ...loginForm, password: e.target.value })} required style={{ padding: '12px', borderRadius: '8px', border: '2px solid #e2e8f0', fontSize: '14px', transition: 'border 0.3s', outline: 'none' }} onFocus={(e) => e.target.style.borderColor = '#0070f3'} />
            ...
            {nextMatches.map(match => {
              ...
              return (
                <div key={match.id} style={{ background: 'white', padding: '1.5rem', borderRadius: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.08)', borderLeft: '4px solid #0070f3' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justify: 'center', gap: '16px', marginBottom: '12px' }}>
